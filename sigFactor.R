...
# WARNING: Due to the vectors not being exactly as expected as described above, some "Inf" and some "NA" will appear in the results.
#          I'm not yet sure of the best way to interpret them.  I suspect "Inf" equates to "very prevalent in the group" and 
#          "NA" equates to "does not exist in the group". Either may indicate relevance.
...
sigFactors <- function(df, group_feature) {
    ...
    for (feature in unique(chunk_gather$enum)) {
        ...
        chunk_cast <- chunk_gather %>% 
            ...
            dcast(list(group_feature, "value"), value.var="sum", fill=0)
        # get cluster as row names
        rownames(chunk_cast) <- chunk_cast[[group_feature]]

        for (group in unique(chunk_gather[[group_feature]])) {

            results[group, feature] <- 0.5 * chi2.plugin(chunk_cast[group, 
                                                                    setdiff(colnames(chunk_cast), group_feature)], 
                                                         colSums(chunk_cast[setdiff(rownames(chunk_cast), group), 
                                                                            setdiff(colnames(chunk_cast), group_feature)]))
            #results[group, feature] <- KL.plugin(chunk_cast[group, 
            #                                                        setdiff(colnames(chunk_cast), group_feature)], 
            #                                             colSums(chunk_cast[setdiff(rownames(chunk_cast), group), 
            #                                                                setdiff(colnames(chunk_cast), group_feature)]))
        }
